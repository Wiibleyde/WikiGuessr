...
export default function Button({
    ...
    return (
        <button
            className={cn(`px-3 py-1.5 rounded-md text-sm font-medium transition-colors cursor-pointer disabled:opacity-20 disabled:cursor-not-allowed ${variants[variant]}`, className)}
            {...props}
        >
            {children}
        </button>
